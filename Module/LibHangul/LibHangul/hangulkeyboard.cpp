...
{
    HangulCombination *combination = new HangulCombination;
    if (combination != NULL) {
	combination->size = 0;
			    ...
			    unsigned int n)
{
    ...
    combination->table = (HangulCombinationItem*)malloc(sizeof(HangulCombinationItem) * n);
    if (combination->table != NULL) {
	int i;
	...
	ucschar first, ucschar second, ucschar result)
{
	...
	table = (HangulCombinationItem*)realloc(table, size_need * sizeof(table[0]));
	if (table == NULL)
	    return false;
...
{
    const HangulCombinationItem *item1 = (const HangulCombinationItem *)p1;
    const HangulCombinationItem *item2 = (const HangulCombinationItem *)p2;
...
hangul_combination_sort(HangulCombination* combination)
{
    ...
    qsort((void**)combination->table, combination->size, sizeof(combination->table[0]), hangul_combination_cmp);
}
			   ...
			   ucschar first, ucschar second)
{
    ...
    res = (HangulCombinationItem *)bsearch(&key, combination->table, combination->size,
...
hangul_keyboard_new()
{
    HangulKeyboard *keyboard = (HangulKeyboard *)malloc(sizeof(HangulKeyboard));
    if (keyboard == NULL)
	return NULL;
...
hangul_keyboard_set_mapping(HangulKeyboard *keyboard, int tableid, unsigned key, ucschar value)
{
	...
	ucschar* new_table = (ucschar* )malloc(sizeof(ucschar) * HANGUL_KEYBOARD_TABLE_SIZE);
	if (new_table == NULL)
	    return;
...
hangul_keyboard_list_clear()
{
    ...
    delete hangul_keyboards.keyboards;
...
hangul_keyboard_list_append(HangulKeyboard* keyboard)
{
	...
	keyboards = (HangulKeyboard** )realloc(keyboards, n * sizeof(keyboards[0]));
	if (keyboards == NULL)
	    return false;
