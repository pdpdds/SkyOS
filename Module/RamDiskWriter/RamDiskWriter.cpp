...
int main(int argc, char** argv) {
	...
	if (argc == 2) {
		if (strcmp(argv[1], "--help") == 0) 
		{
			...
			std::cout << "  SkyOS Kernel Ramdisk Generator"
					<< std::endl;
			std::cout << std::endl;
			std::cout << "DESCRIPTION" << std::endl;
			std::cout
					<< "  This program generates a SkyOS ramdisk from a given source folder."
					<< std::endl;
			std::cout << "  To do so, use the following command syntax:"
					<< std::endl;
			std::cout << std::endl;			
			std::cout << "\texample : ./RamDiskWriter.exe ./source ./target/skyramdisk.pak" << std::endl;
			std::cout << std::endl;
			return 0;
		}
	...
	if (argc == 3) {
		...
		std::cerr << "usage: " << argv[0] << " sourchPath targetPath"
				<< std::endl;
		return 1;
	}
}
...
void SkyRamDisk::Create(const char* sourcePath, const char* targetPath) {
...
void SkyRamDisk::WriteRecursive(const char* basePath, const char* path,
