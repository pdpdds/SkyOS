...
DirEntry  * GetDirectoryEntry(char * FilePath, struct DirEntry * DEInfo)
{
...
#ifdef _DEBUG
	SkyConsole::Print("%s %s %s %s %s\n", FilePath, Drive, Dir, FileName, Ext);
#endif
	...
	if (CurDir[0] == 0)
	{
		...
		SkyConsole::Print("%s %d\n", File, strlen(File));
#endif
		if (EnumerateFilesInRoot(Drive[0], FindFileInEnumeration, File, DEInfo) != 2)
			return 0;
		else
			return DEInfo;
	}
...
{
	SkyConsole::Print("FAT warning :: File Close Not Implemented\n");
}
